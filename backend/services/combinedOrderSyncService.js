...
class CombinedOrderSyncService {
  constructor() {
  ...
  async upsertOrdersToMongo(orders) {
    const results = {
      created: 0,
      updated: 0,
      skipped: 0,
    ...
    for (const order of orders) {
      try {
        ...
        if (!existingOrder) {
          ...
          results.created++;
          console.log(`Created new order in MongoDB: ${order.name} (${order.email})`);
        } else {
          ...
          if (shopifyUpdatedAt > localUpdatedAt) {
            ...
            results.updated++;
            console.log(`Updated order in MongoDB: ${order.name} (${order.email})`);
          } else {
            results.skipped++;
            console.log(`Skipped unchanged order in MongoDB: ${order.name}`);
          }
        }
      } catch (error) {
        console.error(`Error processing order ${order.id} in MongoDB:`, error.message);
        results.errors++;
      }
    }
  ...
  async syncOrders() {
    try {
      console.log('Starting combined order sync from Shopify...');
      ...
      if (!orders || orders.length === 0) {
        ...
        return { 
          mongoResults: { created: 0, updated: 0, skipped: 0, errors: 0 },
          csvResults: { created: 0, updated: 0, skipped: 0, errors: 0 }
        };
      }

      // Sync to MongoDB
      console.log('Syncing orders to MongoDB...');
      const mongoResults = await this.upsertOrdersToMongo(orders);
      
      // Sync to CSV
      console.log('Syncing orders to CSV...');
      const csvResults = await csvOrderSyncService.syncOrders();
      
      console.log(`Combined order sync completed:`, {
        mongodb: {
          created: mongoResults.created,
          updated: mongoResults.updated,
          skipped: mongoResults.skipped,
          errors: mongoResults.errors
        },
        csv: {
          created: csvResults.created,
          updated: csvResults.updated,
          skipped: csvResults.skipped,
      ...
      console.error('Combined order sync failed:', error.message);
      throw error;
    }
  }
...
export default new CombinedOrderSyncService();
